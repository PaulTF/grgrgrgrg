  ...
  main.variable(observer("chart")).define("chart", ["d3","width","height","nodes","groups","forceCluster","forceCollide"], function(d3,width,height,nodes,groups,forceCluster,forceCollide)
{
  // Variables.
  var time_so_far = 0;
  console.log(JSON.stringify(nodes[1].num));
  // The SVG object.
  ...
  svg.selectAll('.grpcnt')
    ...
    .text(d => d3.format(",.0f")(Math.min(groups[d].cnt*7046/1000,groups[d].max)));
  ...
  function timer() {
    ...
    // Update counters.
    svg.selectAll('.grpcnt').text(d => d3.format(",.0f")(Math.min(groups[d].cnt*7046/1000,groups[d].max)));
